...
question.addEventListener("click", () => {
    let chatList = document.querySelector(".chat_content")
        ...
        switch (selectValue) {
            case "que1":
                resultHtml += `<li class="bot_right">한국에서는 대중교통에서 음식물을 먹어도 되나요?</li> 
                <li class="bot_left">한국에서 대중교통에서 음식물을 먹는 것은 허용되지 않습니다. 대부분의 지하철과 버스에서는 음식물 반입과 섭취가 금지되어 있습니다. 단, 물이나 주스 같은 음료는 마실 수 있습니다.
이는 다른 승객들의 불편을 최소화하고 깨끗한 대중교통 환경을 유지하기 위한 규칙입니다. 대중교통 내에서는 음식물 섭취를 자제하시는 것이 좋습니다. 만약 배고프시다면 역이나 정류장에서 먹는 것이 좋습니다. 이 규칙을 지켜주셔서 감사합니다.</li>
 <li class="bot_left">다른 질문은 없으신가요?</li>`;
                document.getElementById("chat_content").innerHTML += resultHtml;
                break;
            case "que2":
                resultHtml += `<li class="bot_right">한국에서 식사할 때 지켜야 할 예절은 무엇인가요?</li> <li class="bot_left">네, 한국에서 식사할 때 지켜야 할 예절이 몇 가지 있습니다:
...
<br> 이러한 예절을 지켜 주시면 한국 문화에 대한 이해도가 높아질 것입니다. 궁금한 점이 더 있으시면 말씀해 주세요.</li> <li class="bot_left">다른 질문은 없으신가요?</li>`;
                document.getElementById("chat_content").innerHTML += resultHtml;
                break;
            case "que3":
                resultHtml += `<li class="bot_right">한국에서 존댓말을 언제 사용해야 하나요?</li> <li class="bot_left">네, 한국에서는 상대방의 나이, 지위, 관계에 따라 존댓말을 사용해야 합니다.
<br> 
...
<br> 
존댓말을 사용하지 않으면 무례하게 보일 수 있으므로, 상황에 맞는 적절한 존댓말 사용이 중요합니다. 궁금한 점이 더 있으시면 말씀해 주세요.</li> <li class="bot_left">다른 질문은 없으신가요?</li>`;
                document.getElementById("chat_content").innerHTML += resultHtml;
                break;
            case "que4":
               resultHtml += `<li class="bot_right">한국에서는 어떤 행동이 무례하다고 여겨지나요?</li> <li class="bot_left">한국에서 다음과 같은 행동들은 무례한 것으로 여겨집니다:
...
<br>이러한 행동들은 한국 문화에서 예의바르지 않은 것으로 여겨집니다. 상대방을 존중하고 배려하는 태도가 중요합니다. 궁금한 점이 더 있으시면 말씀해 주세요.</li> <li class="bot_left">다른 질문은 없으신가요?</li>`;
                document.getElementById("chat_content").innerHTML += resultHtml;
                break;
            case "que5":
                resultHtml += `<li class="bot_right">식당에서 팁을 줘야 하나요?</li> <li class="bot_left">한국에서는 식당에서 팁을 주는 것이 일반적이지 않습니다.
...
다만 해외에서 자주 가던 식당에서는 팁 문화가 자리 잡혀 있을 수 있으니 그런 경우에는 팁을 주는 것이 좋습니다. 궁금한 점이 더 있으시면 말씀해 주세요.</li> <li class="bot_left">다른 질문은 없으신가요?</li>`;
                document.getElementById("chat_content").innerHTML += resultHtml; 
                break;
            default: 
        } 
