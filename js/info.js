...
let underFestivalUrl = new URL(`http://openapi.seoul.go.kr:8088/${festivalApiKey}/json/culturalEventInfo/1/50///2024-07-21`); // API 키
...
const getCardsByCategory = async (event) => { // 버튼을 누르면 해당 카테고리의 행사를 찾아주는 기능
    ...
    if(category ==="전체"){
        underFestivalUrl = new URL(`http://openapi.seoul.go.kr:8088/${festivalApiKey}/json/culturalEventInfo/1/50///2024-07-21`);
        await festivalView();
    }
    else{
...
const cardRender = () => {   // 카드 안에 API 데이터를 넣어서 불러오는 기능
    ...
    const cardsHTML = underFestivalList.map((festival) => 
